...
export default function SettlePage() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      setData({
        session,
        members: readLocalMembers(session),
        expenses: readLocalExpenses(session),
        runs: readLocalSettlementRuns(session),
      });
      setLoading(false);
    }, 0);
  ...
  function closeCurrentPeriod() {
    ...
    const run: SettlementRun = {
      ...
      expenseIds: openExpenses.map((expense) => expense.id),
      transfers,
    };
